...
export const initializeContracts = async (
  ...
  try {
    ...
    if (!networkConfig.contracts.Flip) {
      throw new Error(
        `Flip contract address not configured for ${networkConfig.name}`
      );
    }
    ...
    try {
      ...
      // Create Flip contract instance
      const FlipContract = new ethers.Contract(
        networkConfig.contracts.Flip,
        FlipABI.abi,
        signer
      );
      ...
      return { token: tokenContract, Flip: FlipContract };
    } catch (contractError) {
      throw new Error(
    ...
    if (setContracts) {
      setContracts({ token: null, Flip: null });
    }
    if (setLoadingStates) {
      setLoadingStates(prev => ({ ...prev, contracts: false }));
    }
