...
/**
 * Format Flip roll result for display, handling special result codes
 * @param {Number|String} result - Flip roll result (1-6 or special code)
 * @returns {String} Formatted result
 */
export const formatFlipResult = result => {
  if (!result && result !== 0) return '?';

  const num = Number(result);

  // Regular Flip roll (1-6)
  if (num >= 1 && num <= 6) {
    return num.toString();
  }
