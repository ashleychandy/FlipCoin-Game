...
// Constants
const WALLET_STORAGE_KEY = 'xdc_Flip_wallet_connection';
...
export const WalletProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!mainnetConfig?.contracts?.Flip) {
      if (DEFAULT_NETWORK === 'mainnet') {
        addToast(
          'Mainnet Flip contract is not configured. Some features may not work.',
          'warning'
        );
      }
    }
    ...
    if (!testnetConfig?.contracts?.Flip) {
      if (DEFAULT_NETWORK === 'apothem') {
        addToast(
          'Testnet Flip contract is not configured. Some features may not work.',
          'warning'
        );
      }
    }
  ...
  const enhancedWalletState = useMemo(() => {
    return {
      ...
      getFlipContract: () => {
        if (!walletState.contracts?.Flip) {
          return null;
        }
        return walletState.contracts.Flip;
      },
