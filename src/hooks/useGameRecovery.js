...
export const useGameRecovery = ({ onSuccess, onError } = {}) => {
  ...
  const { contract: FlipContract } = useFlipContract();
  ...
  const {
    ...
    mutationFn: async () => {
      if (!account || !FlipContract) {
        throw new Error('Wallet not connected or contract not initialized');
      }

      const tx = await FlipContract.recoverOwnStuckGame();
  ...
  const checkRecoveryEligibility = useCallback(
    async playerAddress => {
      ...
      if (playerAddress && playerAddress !== account) {
        if (!FlipContract) return null;

        try {
          const status = await FlipContract.getGameStatus(playerAddress);
    ...
    [FlipContract, account, gameStatus]
  );
